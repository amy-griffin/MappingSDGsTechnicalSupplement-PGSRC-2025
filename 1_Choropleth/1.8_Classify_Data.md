...
We don't want QGIS to consider NULL values when it creates a histogram of our data values. So we will apply a filter so that it does not include these values.

In the *Layer Properties* window, go to the *Source* tab and click the *Query Builder* button.

Double-click the *unwp_2025* field from the list of *Fields*.

Then type a space followed by *IS NOT NULL* and click *OK*.

If you examine the map, you'll see some countries appear to have disappeared. This means our filtering has worked.


**Setting the Colour Ramp**

Back in the *Symbology* tab, change *Single symbol* to *Graduated* at the top of the screen.

Under *Value* select *unwp_2025*.
...
Scroll down to *Symbol*. Under *Color* of the *Fill* select a light grey, commonly used to represent Missing Data. Also change the *Stroke color* to light grey.

Click *OK* to add these conditions.

Finally, return to the *Query Builder* in the *Source* tab and remove the filter we applied previously by clicking the *Clear* button followed by *OK*, and then click *OK* again to close the *Properties* window and apply all changes.
